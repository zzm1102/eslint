...
SourceCode.prototype = {
    ...
    getComments(node) {
        ...
        const isComment = token => token.type === "Line" || token.type === "Block";
        ...
        // Attach comments as children comments if a node contains only comments.
        if (
            (node.type === "Program" || node.type === "BlockStatement" || node.type === "ClassBody") && node.body.length === 0 ||
            node.type === "ObjectExpression" && node.properties.length === 0 ||
            node.type === "ArrayExpression" && node.elements.length === 0
        ) {
            const tokens = this.getTokens(node);

            attachedComments.children = tokens.filter(isComment);
        }

        // The top level program node should not have leading or trailing comments.
        if (node.type !== "Program") {
            const isPunctuator = token => token.type === "Punctuator";
            let currentToken = this.getTokenOrCommentBefore(node);

            while (currentToken && (isComment(currentToken) || isPunctuator(currentToken))) {
                if (isPunctuator(currentToken)) {
                    const tokenNode = this.getNodeByRangeIndex(currentToken.start);

                    // Stop looping when we have reached the previous node or the beginning of the
                    // parent node to avoid duplicate comment attachment
                    if (tokenNode.end < node.start || currentToken.start <= node.parent.start) {
                        break;
                    }
            ...
            while (currentToken && (isComment(currentToken) || isPunctuator(currentToken))) {
                if (isPunctuator(currentToken)) {
                    ...
                    // Stop looping when we have reached the next node or the end of the
                    // parent node to avoid duplicate comment attachment
                    if (tokenNode.start > node.end || currentToken.end >= node.parent.end) {
                        break;
                    }
                } else if (isComment(currentToken)) {
